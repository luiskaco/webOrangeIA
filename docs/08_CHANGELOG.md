...
### Cambiado
- **Sincronización de SEO Agéntico / GEO:** Actualización integral de `llms.txt`, paquete `okf/` (Open Knowledge Format) y Schema JSON-LD `@graph` (`inc/seo.php` y `page-gestion-de-acceso.php`) con los nuevos alcances y nomenclatura de servicios.
...
- **Bumping de Versión del Tema:** Versión elevada a `1.2.2` en `style.css` y `functions.php` para despliegue y cache busting en hosting.
